...
/*   Updated: 2024/04/26 16:11:44 by asfletch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (point.y + 1 < cube->max_height && cube->map->map[point.y + 1][point.x] != '1' && cube->map->map[point.y + 1][point.x] != 'V')
	{
		stack[*top] = get_point(point.x, point.y + 1);
		(*top)++;
	}
	if (point.y > 0 && cube->map->map[point.y - 1][point.x] != '1' && cube->map->map[point.y - 1][point.x] != 'V')
	{
		stack[*top] = get_point(point.x, point.y - 1);
		(*top)++;
	}
...
{
	if (point.x + 1 < cube->max_width && cube->map->map[point.y][point.x + 1] != '1' && cube->map->map[point.y][point.x + 1] != 'V')
	{
		stack[*top] = get_point(point.x + 1, point.y);
		(*top)++;
	}
	if (point.x > 0 && cube->map->map[point.y][point.x - 1] != '1' && cube->map->map[point.y][point.x - 1] != 'V')
	{
		stack[*top] = get_point(point.x - 1, point.y);
		(*top)++;
	}
